...
4. 對非平凡切片，先寫出讓 done 可驗證的 sprint contract。
5. 選擇一項可觀察行為，完成一個 red-green-refactor 垂直切片。
6. 使用可重現的證據驗證測試、架構、文件與執行階段行為。
7. Review 規格符合度、harness 品質、架構漂移與 reviewer 品質。
8. 提交完整成果、在流程需要時 push，並只在仍有工作時留下精簡 handoff。
